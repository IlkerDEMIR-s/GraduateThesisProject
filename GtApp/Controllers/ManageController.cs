...
namespace GtApp.Areas.Admin.Controllers
{
    public class ManageController : Controller
    {
        ...
        public ManageController(IServiceManager manager, UserManager<IdentityUser> userManager)
        {
            _manager = manager;
            _userManager = userManager;
        }

        private async Task<Author?> loggedInUser()
        {
            var user = await _userManager.GetUserAsync(User);

            string userId = user.Id;

            var author = _manager.AuthorService.GetAuthorByAspNetUserId(userId, false);
            return author;
        }
        ...
        {
            Author? author = await loggedInUser();

            ViewData["Title"] = "Manage Theses";
            var theses = _manager.ThesisService.GetAllTheses(false)
                     .Where(t => author != null && t.AUTHOR_ID == author.AUTHOR_ID)
                     .ToList();
            ...
            foreach (var thesis in theses)
            {
                ...
                var newAuthor = authorship != null ? _manager.AuthorService.GetOneAuthor(authorship.AUTHOR_ID, false) : null;
        ...
        public async Task<IActionResult> Create([FromForm] ThesisViewModel viewModel)
        {
            Author? author = await loggedInUser();
            ...
            if (ModelState.IsValid)
            {
                ...
                thesis.AUTHOR_ID = author.AUTHOR_ID;
